...
germ <- read_csv("light_round1_tall.csv") %>%
  select(pops, temps, wps, total_seeds, germ, day, cumulative_germ)
germ
```
...
germone <- germ %>% group_by(wps, temps) %>%
...
### 2) Try incorporating water potential

```{r}
d <- list(N=nrow(germone),
          germ=germone$germ,
          temps=as.numeric(as.factor(germone$temps)),
          wps=as.numeric(as.factor(germone$wps)),
          day=germone$day)
...
parameters{
    ...
    vector[8] a; // alpha for the gamma curve, one for each temp
    vector[8] ap; // alpha for the proportion dormant, one for each temp
    vector[3] b; // b for the gamma curve, one for each wps
    vector[3] bp; // b for the proportion dormant, one for each wps
}
model{
    vector[N] p;
    vector[N] mu;
    ...
    shape ~ exponential(.5); 
    for (i in 1:N) {
        p[i] = ap[temps[i]]+bp[wps[i]];
        p[i] = inv_logit(p[i]);
    }
    for ( i in 1:N ) {
        mu[i] = a[temps[i]]+b[wps[i]];
         // apply the inverse link function
        mu[i] = shape * exp(-(mu[i]));
    }
    for ( i in 1:N ) 
