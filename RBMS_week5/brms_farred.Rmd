...
farred %>% group_by(genotype, concentration) %>%
  ...
  scale_x_log10() +
  geom_jitter(alpha=.2, data=farred)
```
...
```{r}
farred.small <- farred %>% filter(concentration >0)
farred.small %>% group_by(genotype, concentration) %>%
  summarize(mlength=mean(length, na.rm=TRUE), 
            sem=sem(length),
            ymin=mlength-sem, 
            ymax=mlength+sem) %>%  
  ...
  geom_errorbar(aes(ymin=ymin, ymax=ymax), width=.1) +
  scale_x_log10() +
  geom_jitter(alpha=.2, data=farred.small)
```
...
farred.small %>% group_by(genotype, concentration) %>%
  ...
  geom_violin() +
  geom_errorbar(aes(ymin=ymin, ymax=ymax), width=.1) +
  geom_jitter(alpha=.5, data=farred.small)
```

## with brms
```{r}
farred.small <- farred.small %>% mutate(lc = log10(concentration))
